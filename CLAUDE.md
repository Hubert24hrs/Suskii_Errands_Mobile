...
| Phase 4 — Verification & safety | **Started 2026-09-21**, on ADR-0013/ADR-0014's terms. Done: safety (`…20260921120000_safety`) — `trusted_contacts` (five, by trigger; numbers encrypted per ADR-0007), `sos_incidents` + `sos_partners` with the ops queue verbs and a three-minute escalation (AD-21), and `trip_share_links` whose token is stored only as a hash, with `get_shared_trip` as the second and last `anon`-callable function; 40 pgTAP assertions. Next in this phase: the KYC spine (verification sessions, steps, the Verification Officer queue, document expiry and auto-suspension), restricted items and moderation rules, then the risk engine. Blocked: the identity vendor itself (Smile ID contract, OD-13) and the SOS partner integration (client action 12) — both are seams here, not integrations |
