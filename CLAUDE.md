...
| Phase 3 — Marketplace core | **Started 2026-09-18** (ADR-0014, on ADR-0013's terms). Done: service taxonomy (`service_categories` + dev seed) and `requests` — `create_request`, `publish_request`, `cancel_request`, `private.expire_requests`, RLS per matrix §5 (draft-only edits by column grant, no provider table access), 24 pgTAP assertions, `request_channel` enum and codes `ERR_REQUEST_NOT_FOUND` / `ERR_CATEGORY_NOT_FOUND` in the preview. Next: offer threads and negotiation (TTL, rounds, alternation, row locks per S-10), then PostGIS matching, then the job state machine. Money-dependent rules wait for OD-06/OD-08/OD-19 |
