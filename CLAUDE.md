...
| Phase 3 — Marketplace core | **Started 2026-09-18** (ADR-0014, on ADR-0013's terms). Done: service taxonomy (`service_categories` + dev seed) and `requests` — `create_request`, `publish_request`, `cancel_request`, `private.expire_requests`, RLS per matrix §5 (draft-only edits by column grant, no provider table access), 26 pgTAP assertions, `request_channel` enum and codes `ERR_REQUEST_NOT_FOUND` / `ERR_CATEGORY_NOT_FOUND` in the preview. Offers and negotiation (2026-09-18, `…120100_marketplace_offers`): `offer_threads`, `offers`, `pricing_guardrails`; `create_offer`, `counter_offer`, `accept_offer`, `decline_offer`, `withdraw_offer`, `private.expire_offers`; alternation, per-thread round limit, country hard-max guardrail, the S-10 acceptance transaction (lock, accept, expire siblings, close threads, one event) and the competitive invariant in RLS; 46 pgTAP assertions. Contracts preview bumped to 1.0.0-preview.2 (55 codes, 21 enums). Deviations from the Phase 1 drafts, recorded in those documents: counters and accepts name an offer rather than a thread, the counterparty accepts (a customer's counter is otherwise unacceptable), withdrawal leaves the thread open. Providers and matching (2026-09-18, `…120200_providers_and_matching`): `provider_profiles`, `provider_services`, `provider_service_areas`, `provider_live_location`; `set_online`, `update_provider_services`, `update_provider_service_areas`, `heartbeat` (movement-gated per S-06), `provider_feed` (single radius, own country, registered categories, **distance not coordinates**) and `private.match_providers` (weighted score, weights in `remote_config`); suspension now guards `create_offer` and `accept_offer` too; 39 pgTAP assertions. Jobs (2026-09-18, `…120300_jobs`): `jobs`, `job_events` (append-only, monthly partitions, forced RLS per partition), `private.job_pins`; `set_job_status`, `verify_pin`, `reveal_job_pin`, `confirm_completion`, `private.mark_paid_held` (the payment seam, no client grant), `private.auto_confirm_jobs`; transitions 6, 8, 11–17 and 23 of the job lifecycle; 46 pgTAP assertions. **Phase 3 is now complete except for what money blocks**: payment, settlement, refunds, disputes and cancellation fees all wait for OD-06/OD-08/OD-19, and price guardrail calibration is OD-23. Proofs and media (2026-09-18, `…120400_proofs_and_media`): buckets `request-media` and `job-proofs`, `public.proofs`, `submit_proof`, and proof completeness from the category's `proof_requirements` enforced by trigger; 25 pgTAP assertions. Open on my side: EXIF stripping has no worker yet, so clients strip before upload. Ratings and reputation (2026-09-18, `…120500_ratings_and_reputation`): `ratings` with the SH-30 blind period, `rate_job`, `private.close_rating_windows`, `private.recompute_reputation` (Bayesian, visible ratings only) feeding the matching score; **`provider_feed` replaced** to fix my own PR-11 breach — it returned the exact pickup address, and now returns city + a point rounded to ~1 km + the photos; 32 pgTAP assertions. Chat and channels (2026-09-18, `…120600_chat_and_realtime`): `conversations`, `messages`, `message_reads`, `chat-media`, `send_message`, `mark_read`, `private.may_join_topic` and the `realtime.messages` policies (bound, asserted in CI; the join path itself is [A] until S-02); 33 pgTAP assertions. **Phase 3 backend is complete but for money and vendors**: payment, settlement, refunds, disputes and cancellation fees wait on OD-06/OD-08/OD-19; calls and masked numbers need a vendor; notifications need the push work. Next: contracts v1 Stage B at M8.5, or the SOS and safety tables |
| Next Claude work | Phase 2 build complete: deploy pipelines exist but are switched off until the client's GCP projects, Supabase org and GitHub environments exist (RB-14 checklist). Phase 3 is under way (ADR-0014); rolling review of Kimi's milestones (recheck C.1, M3.1 at M3 commit); Stage B at M8.5. Launch base case: week of 7 Jun 2027 |
