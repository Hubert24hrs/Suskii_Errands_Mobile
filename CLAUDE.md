...
| Next Claude work | Phases 2–4 are built; Phase 5 (money) is next and is blocked on client answers, not on code: OD-06 (commission rate), OD-08 (gateway fee on refunds), OD-12 (funds holding model) and OD-19. Deploy pipelines exist but stay switched off until the client's GCP projects, Supabase org and GitHub environments exist (RB-14 checklist). Buildable meanwhile: Phase 6 call metadata (`calls`, `masked_numbers` — metadata only, no recording column) and Phase 8 admin backend. Rolling review of Kimi's milestones; Stage B at M8.5. Launch base case: week of 7 Jun 2027 |
