...
|---|---|---|
| [`rpc-catalog/index.json`](rpc-catalog/index.json) | All **122** functions a client may call: arguments with types and defaults, return shape, the error codes each can raise, whether it needs an idempotency key, and what it requires of the caller | **Generated** from `supabase/migrations` |
...
## The nine things most likely to bite
...
3. **62 of the 122 functions take an idempotency key**, and it is the *first* argument on every
...
5. **Column grants are narrower than table grants on 5 tables.** `promo_codes` is the one to
...
7. **`payments.checkout_url` is not readable.** It is a capability, not a reference, and a job's
   provider could read the customer's. It lives behind `get_payment_checkout(request_id)`, which
   answers the payer only. **`select('*')` on `payments` fails** -- name the columns.
8. **A provider reads a request only once the job is funded.** `provider_feed` gives an
...
9. **Realtime is a courtesy, not a guarantee.** Tables are the truth. Refetch on reconnect.
