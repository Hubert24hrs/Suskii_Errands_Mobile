...
|---|---|---|---|---|---|
| 1 | — | `ACTIVE` | Provider | Provider eligible; request `PUBLISHED`/`OFFERS_RECEIVED`/`NEGOTIATING`; **not self-dealing**; amount within guardrails; provider has no unresolved blocking job | Notify customer; request → `OFFERS_RECEIVED`; start TTL |
| 2 | `ACTIVE` | `COUNTERED` | C or P | Round count < max (default 5); author is the counterparty, not the same side twice | New `ACTIVE` offer in the thread; request → `NEGOTIATING`; TTL resets |
| 3 | `ACTIVE` | `ACCEPTED` | Customer | Offer `ACTIVE` and unexpired; provider still eligible; request in an acceptable state | **All sibling offers → `EXPIRED` in the same transaction**; request → `AGREED`; commission rate and amount snapshotted |
| 4 | `ACTIVE` | `REJECTED` | Customer | — | Notify provider; thread closed, others unaffected |
| 5 | `ACTIVE` | `WITHDRAWN` | Provider | Not yet accepted | Notify customer if it was the last active offer |
| 6 | `ACTIVE` | `EXPIRED` | System | TTL elapsed | If no `ACTIVE` offers remain, request → `PUBLISHED` |
| 7 | `ACTIVE` | `EXPIRED` | System | A sibling was accepted (#3) | Emitted as part of the accept transaction, not separately |
| 8 | `ACTIVE` | `EXPIRED` | System | Provider became ineligible (suspended, documents expired, went offline past grace) | Notify provider with the reason |

Terminal states are `ACCEPTED`, `REJECTED`, `WITHDRAWN`, `EXPIRED`, `COUNTERED`.
...
- **Round limit** (default 5) counts offers in a thread, both directions. On the limit, only accept, reject or withdraw remain.
...
| Reject | `reject_offer(offer_id, idempotency_key)` | Customer only |
...
3. Should accepting auto-reject the other threads, or expire them? Expire, as above — "rejected" implies a customer judgement that did not happen.
