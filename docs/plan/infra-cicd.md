...
| `security.yaml` | All pushes and PRs; nightly | CodeQL (Python, JS/TS), Semgrep (`p/owasp-top-ten`, `p/secrets` + custom rules: no `SECURITY DEFINER` without `SET search_path = ''`, no `float`/`numeric` money columns, no `service_role` in `apps/`), dependency review, `osv-scanner` on lockfiles, **ADR-0010 banned-package check** on Dart and npm lockfiles | Yes (high and critical) — **implemented 2026-09-16**: `infra/policy/policy_check.py` (ADR-0002 copy, ADR-0010 packages, client secrets, Android permissions; unit-tested), gitleaks over full history, osv-scanner on committed lockfiles, weekly schedule. CodeQL and dependency review wait for GitHub Advanced Security (private repo, I-1); Deno's lockfile is not yet scannable |
